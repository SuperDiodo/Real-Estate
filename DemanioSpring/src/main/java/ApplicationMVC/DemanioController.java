...
public class DemanioController {
	...
	/**
	 * 
	 * @return Ritorna i metadati dell'oggetto Concessione in formato JSON con un
	 *         codice HTTP 200.
	 */
	...
	public ResponseEntity filters(@RequestParam String filter) throws JSONException {
		...
		if (vett.size() == 0) return new ResponseEntity<String>("Non ci sono stati risultati della ricerca", HttpStatus.NOT_FOUND);
	...
	/**
	 * Restituisce le statistiche di tutta la collezione in base all'attributo
	 * passato Gli attributi possibili sono: - "nome", "RagSoc", "IDCom", "cognome",
	 * "comune", "den" [STRINGHE] - "superficie","supwater","durata" [NUMERICI]
	 * 
	 * Di default un filtro è vuoto,immettendolo è possibile ottenere statistiche di
	 * una selezione della collezione
	 * 
	 * @param field  campo su cui calcolare le stats
	 * @param filter eventuale filtro da applicare
	 * @return In base al tipo di risposta avremo dei risultati: 1) OK: hash map dei
	 *         risultati. 2) NOT FOUND: stringa di errore, non sono trovate
	 *         statistiche. 2) BAD REQUEST: stringa di errore, nel caso in cui il
	 *         campo richiesto o il filtro non siano stati scritti correttamente
	 * @throws JSONException
	 */
	@SuppressWarnings("rawtypes")
	@GetMapping("/stats")
