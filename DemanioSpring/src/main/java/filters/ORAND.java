 ...
 */
	...
	//esempio query string den: http://localhost:8080/filtering?filter={"type":"and", "fields": "den","cities":%5B "sponda","tevere" %5D}
	//esempio query string search: http://localhost:8080/filtering?filter={"type":"and", "fields": "search","cities":%5B "roma",6935 %5D}
	/**
	 * @param elem elemento da controllare.
	 * @param equal  equal = true viene applicato il filtro "or".
	 *               equal = false viene applicato il filtro "and" tra elementi dello stesso tipo.
	 *               Inoltre è stato aggiunto nello switch case il caso "search" che implementa il filtro "and" tra il tipo comune ed il tipo durata per cercare una 
	 *               concessione in un dato comune e con una certa durata. 
	...
	public boolean approved(Concessione elem, boolean equal) throws JSONException {
		...
		case "search":
			approvato = true;
				if (!(elem.getComune().equalsIgnoreCase(cities.get(0).toString()) && elem.getDurata() == Integer.parseInt(cities.get(1).toString()))) {
					approvato = false;
				break;
				}
		}
