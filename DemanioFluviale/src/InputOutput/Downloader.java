...
public class Downloader {
	...
	/**
	 * partendo dal file descrittivo del data-set ricerca l'url assoluto del file da
	 * scaricare
	 * 
	...
	public boolean RicercaFile() throws IOException, JSONException {
		...
		for (int i = 0; i < array.length(); i++) {
			...
			if (temp_url.contains(getExt())) {
				setFile_url(array.getJSONObject(i).getString("url"));
				return true;
			}
		}
	...
	/**
	 * converte, se necessario, l'url immesso in un file csv. Controlla se il file è
	 * già esistente.
	 * 
	...
	public void url2file(String filename) throws MalformedURLException, IOException, JSONException {
		...
		if (new File(filename).exists()) {
			System.out.println("Non serve effettuare il download, file gia' presente\n");
			return;
		}
		;
		...
		if (RicercaFile()) {
			...
			System.out.println("Download non effettuato\n");
	}
